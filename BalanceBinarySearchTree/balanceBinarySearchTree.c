...
/* 计算结点的平衡因子 */
static int AVLTreeNodeBalanceFator(AVLTreeNode *node);
/* 判断结点是否平衡 */
...
static int AVLTreeNodeBalanceFator(AVLTreeNode *node)
{
    ...
    int rightHeight = node->right == NULL ? 0 : node->left->height;
...
{
    #if 1
    return abs(AVLTreeNodeBalanceFator(node)) <= 1;
    #else
    int nodeoFator = abs(AVLTreeNodeIsBalanced(node));
    if (nodeoFator <= 1)
    {
        return true;
    }
...
static int AVLTreeCurrentNodeRotateRight(BalanceBinarySearchTree *pBstree, AVLTreeNode *grand)
{
    ...
    AVLTreeNodeUpdateHeight(parent);
#else
    return AVLTreeNodeRotate(pBstree, grand, parent, child);
#endif
    return ret;
}
...
static int AVLTreeNodeUpdateHeight(AVLTreeNode *node)
{
    ...
    return 1 + maxAVKTreeNodeHeight(leftHeight, rightHeight);
}
