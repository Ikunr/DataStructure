...
/* 静态函数只在本源文件(.c)使用 */
/* 静态前置声明 */
static int DoubleLinkListAccordAppointValGetPos(DoubleLinkList * pList, ELEMENTTYPE val, int *pPos, int (*campareFunc)(ELEMENTTYPE, ELEMENTTYPE));
/* 封装新增节点 */
static DoubleLinkNode * createDoubleLinkListNode(ELEMENTTYPE val);
...
#if 1
    /* 新建新节点封装成函数 */
    DoubleLinkNode * newNode = createDoubleLinkListNode(val);
#else
    /* 封装结点 */
    DoubleLinkNode * newNode = (DoubleLinkNode *)malloc(sizeof(DoubleLinkNode) * 1);
    if (newNode == NULL)
...
int DoubleLinkListDelAppointPos(DoubleLinkList * pList, int pos)
{
    ...
    if (pos == pList->len)
    {
        /* 在末尾位置 */
        /* 备份尾指针 */
        DoubleLinkNode *tempNode = pList->tail;
        /* 移动尾指针 */
...
static int DoubleLinkListAccordAppointValGetPos(DoubleLinkList * pList, ELEMENTTYPE val, int *pPos, int (*campareFunc)(ELEMENTTYPE, ELEMENTTYPE))
{
    ...
    {
        cmp = campareFunc(travelNode->data, val);
        if(cmp == 0)
...
/* 链表删除指定的数据 */
int DoubleLinkListDelAppointData(DoubleLinkList * pList, ELEMENTTYPE val, int (*campareFunc)(ELEMENTTYPE, ELEMENTTYPE))
{
    int ret = 0;
    /* 元素在链表中的位置 */
    int pos = 0;
    ...
    {
        /* 根据指定的元素得到在链表中所在的位置 */
        DoubleLinkListAccordAppointValGetPos(pList, val, &pos, campareFunc);
